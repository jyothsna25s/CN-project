#ifndef __sastra_pkt_h__
#define __sastra_pkt_h__

#include <packet.h>

#define HDR_SASTRA_PKT(p) hdr_sastra_pkt::access(p)

struct hdr_sastra_pkt {
    ...
    inline static hdr_sastra_pkt* access(const Packet* p) {
    	return (hdr_sastra_pkt*)p->access(offset_);
    }
};
